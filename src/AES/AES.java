...
public class AES {
    ...
    static int[] rotateLeft(int[] row, int n) {
        int[] res = new int[4];
        for (int i = 0; i < 4; i++)
            res[i] = row[(i + n) % 4];
        return res;
    ...
    static void addRoundKey(int[][] state, int round) {
        for (int c = 0; c < 4; c++) {
            for (int r = 0; r < 4; r++) {
    ...
    static int[] fromState(int[][] state) {
    ...
    static int[] encrypt(int[] plaintext, int[] key) {
        roundKeys = AESKeyExpansion.keyExpansion(key);
